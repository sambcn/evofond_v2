...
from back.utils import Y_MIN, G, get_matrix_max, read_hecras_data, reverse_data, time_to_string, add_injections
...
class Profile():
    ...
    def compute_event(self, hydrogram, t_hydrogram, law, sedimentogram=None, backup=False, debug=False, method="ImprovedEuler", friction_law="Ferguson", cfl=1, dt=None, dt_save=None, critical=False, upstream_condition="normal_depth", downstream_condition="normal_depth", plot=False, animate=False, injection_list=None):
        """
        ...
        while t <= tf:
            ...
            Q_list = add_injections(Q_list, Q_injected)
        ...
        if plot:
            ...
            for i, index in enumerate(index_list):
                axs[0].axvline(x=x[index], label=f"injection number {i+1} at x={x[index]:.1f}m")
